  ...
  return (
    <>
      <ul role='list' className='divide-y divide-border'>
        {data?.rows.map((practice: Practice) => (
          <li key={practice.id}>
            <Link href={'/student/practices/' + practice.id} className='block hover:bg-muted'>
              <div className='px-4 py-4 sm:px-6'>
                <div className='flex items-center justify-between'>
                  <p className='text-sm font-medium text-foreground truncate'>{practice.type.name} практика
                    #{practice.id}</p>
                ...
                <div className='mt-2 sm:flex sm:justify-between'>
                  <div className='sm:flex text-accent-foreground'>
                    <p className='flex items-center text-sm'>
                      ...
                      {practice.institute ? practice.institute.name : 'Не указано'}
                    </p>
                    <p className='mt-2 flex items-center text-sm sm:mt-0 sm:ml-6'>
                      ...
                      {practice.speciality ? practice.speciality.name : 'Не указано'}
                    </p>
                  </div>
                  <div className='mt-2 flex items-center text-sm text-muted-foreground sm:mt-0'>
                    ...
                    <p>
                      <time dateTime={practice.start_date}>{format(practice.start_date, 'PPP', { locale: ru })}</time>
                      {' '}&mdash;{' '}
                      <time dateTime={practice.end_date}>{format(practice.end_date, 'PPP', { locale: ru })}</time>
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className='flex items-center justify-end px-2 pb-4 mt-4'>
        <div className='flex items-center space-x-6 lg:space-x-8'>
          ...
          {data && data.meta.totalCount / queryOptions.paginationOptions!.pageSize! > 0 &&
            <>
